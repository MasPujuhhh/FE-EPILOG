<template>
          ...
          >
            ...
            >
              <div
                ...
                class="header-form"
              >
                <h2 style="font-weight: bold">Edit Certificate</h2>
              </div>
              <!-- form content -->
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      >
                        <option selected>{{ company_role }} | {{ email }} / {{ fullname }}</option>
  ...
  <script setup>
    ...
    let judul = ref("" || item.value.judul);
    let user_id = ref("")
    let file_pdf = ref("")
    ...
    watch(item, (newValue, oldValue) => {
        // console.log(item.value[0].user_id)
      judul.value = item.value[0].title.split('|')[0]
      user_id.value = item.value[0].user_id
      file_pdf.value = item.value[0].file_pdf
      company_role.value = item.value[0].company_role
      email.value = item.value[0].email
      fullname.value = item.value[0].fullname
    });
    ...
    const viewCertificate = async () => {
      try {
        let res = await axios.get(`${endpoint}/certificate/getCertificateById/${route.params.id}`, {
          headers: {
        ...
        let resp = await axios.get(`${endpoint}/user/getDataOnlyUser`, {
          headers: {
            Authorization: `Bearer ${valid}`,
            "Content-Type": "application/json",
          },
      ...
      try {
        ...
        if (!user_id.value) {
          ...
          let res = await axios.put(
            `${endpoint}/certificate/updateCertificate/${route.params.id}`,
            formData,
            {
              headers: {
                Authorization: `Bearer ${valid}`,
